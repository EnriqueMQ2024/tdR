litterman_W <- function(Y, 
  ...
  ' See master function litterman() for detailed information'
  ...
  # Optimization of objective function (Likelihood or WLS)
  nrl <- length(rl)
  ...
  switch(opp1,
         '0' = {
           rl <- c(0.05, 0.99, 50)
           rex <- criterion_L(Y, x, ta, type_estim, opC, rl, X, C, N, n)
           rho <- rex$rho
           r <- rex$r
         },
         '1' = {
